"""
Visualization script for GRU results
...
def plot_roc_by_race(y_true, y_pred, races, save_path="roc_by_race.png"):
    ...
    plt.xlabel('False Positive Rate', fontsize=12, fontweight='bold')
    plt.ylabel('True Positive Rate', fontsize=12, fontweight='bold')
    plt.title('ROC Curves by Demographic Group - GRU', fontsize=14, fontweight='bold')
    plt.legend(loc="lower right", fontsize=11)
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
...
def plot_performance_by_race(predictions, labels, races, save_path="performance_by_race.png"):
    ...
    ax.set_title('Performance Metrics by Demographic Group - GRU', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
...
def create_results_table(predictions, labels, races, save_path="results_table.txt"):
    ...
    with open(save_path, 'w') as f:
        f.write("="*70 + "\n")
        f.write("GRU MODEL RESULTS - GLAUCOMA DETECTION\n")
        f.write("="*70 + "\n\n")
...
def main():
    print("="*60)
    print("CREATING VISUALIZATIONS FOR GRU MODEL")
    print("="*60)
    
    os.makedirs("figures_gru", exist_ok=True)
    ...
    plot_roc_curve(labels, predictions, 
                   title="ROC Curve - GRU Model",
                   save_path="figures_gru/roc_curve.png")
    
    plot_roc_by_race(labels, predictions, races,
                     save_path="figures_gru/roc_by_race.png")
    
    plot_confusion_matrix(labels, predictions,
                         save_path="figures_gru/confusion_matrix.png")
    
    plot_performance_by_race(predictions, labels, races,
                            save_path="figures_gru/performance_by_race.png")
    
    print("\n[4/4] Creating results table...")
    create_results_table(predictions, labels, races,
                        save_path="figures_gru/results_table.txt")
    
    print("\n" + "="*60)
    print("✓ All GRU visualizations created successfully!")
    print("="*60)
    print("\nGenerated files in 'figures_gru/' directory:")
    print("  - roc_curve.png")
