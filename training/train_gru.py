...
Uses best configuration from LSTM experiments (256 hidden units)
"""
...
# Best configuration (from LSTM experiments)
EMBEDDING_DIM = 300
HIDDEN_SIZE = 256  # Using best config
NUM_LAYERS = 2
DROPOUT = 0.3
BATCH_SIZE = 32
LEARNING_RATE = 0.001
...
def main():
    print("="*60)
    print("TRAINING GRU MODEL FOR GLAUCOMA DETECTION")
    print("="*60)
    ...
    print("\n[3/5] Building vocabulary and datasets...")
    word2idx, vocab = build_vocabulary(train_texts, min_freq=2)
    vocab_size = len(vocab)
    print(f"Vocabulary size: {vocab_size}")
    
    train_dataset = GlaucomaDataset(train_texts, train_df['label'].values, train_df['race'].values, word2idx, MAX_SEQ_LENGTH)
    val_dataset = GlaucomaDataset(val_texts, val_df['label'].values, val_df['race'].values, word2idx, MAX_SEQ_LENGTH)
    test_dataset = GlaucomaDataset(test_texts, test_df['label'].values, test_df['race'].values, word2idx, MAX_SEQ_LENGTH)
    
    train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=BATCH_SIZE, shuffle=False)
    test_loader = DataLoader(test_dataset, batch_size=BATCH_SIZE, shuffle=False)
    
    print("\n[4/5] Creating GRU model (256 hidden units)...")
    model = GRUClassifier(
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_auc > best_val_auc:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), 'best_gru_256.pth')
    ...
    model.load_state_dict(torch.load('best_gru_256.pth'))
    test_loss, test_auc, test_sens, test_spec, predictions, targets, races = evaluate(model, test_loader, criterion, device)
